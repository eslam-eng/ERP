...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            @if(auth()->user()->hasPermission('create_product_addrequest'))
                <h1>
                    <a href="{{route('productaddrequest.create')}}" role="button" class="btn btn-primary"><i class="fa fa-plus"></i> {{trans('trans.product_addrequest')}}</a>
                </h1>
            @endif
            <ol class="breadcrumb">
                            ...
                            <table id="example" class="table table-bordered table-striped">
                                ...
                                @foreach($products as $product)
                                    <tr>
                                        ...
                                        <td>{{$product->note}}</td>
                                        <td>
                                            @if(auth()->user()->hasPermission('update_product_addrequest'))
                                                <a href="{{route('productaddrequest.edit',$product->id)}}" role="button" class="btn btn-warning"><i class="fa fa-edit"></i></a>
                                            @endif
                                            @if(auth()->user()->hasPermission('delete_product_addrequest'))
